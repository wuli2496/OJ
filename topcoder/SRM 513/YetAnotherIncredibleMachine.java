/**
 * Problem Statement
 *     	You may remember an old computer game called "The Incredible Machine". It was a game where you could simulate simple processes like balls falling, lasers shooting, or cats pursuing mice. Manao is faced with the following problem in this game.
 *
 *
 *
 ...
 * Returns:	int
 * Method signature:	int countWays(int[] platformMount, int[] platformLength, int[] balls)
...
class YetAnotherIncredibleMachine
{
    public int countWays(int[] platformMount, int[] platformLength, int[] balls)
    {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int ball : balls) {
                if (mount >= ball) {
                    pmin = Math.max(ball + 1, pmin);
                }

                if (mount <= ball) {
                    pmax = Math.min(ball - 1 - len, pmax);
                }
            }
